...
std::string Maze::getMaze() {
	//Utility function to turn maze into a string of numbers
	std::string strmaze = ""; //First create an empty string
	//Fill upper walls
	for (int x = 0; x < xsize + 2; x++) {
		strmaze += "1 ";
	}
	strmaze += "\n";
	//Concatenate each maze cell value (int) converted to string into maze string 
	for (int y = 0; y < ysize; y++) {
		strmaze += "1 ";
		for (int x = 0; x < xsize; x++) {
			strmaze += std::to_string(maze[y][x].getVal());
			strmaze += " ";
		}
		strmaze += "1\n";
	}
